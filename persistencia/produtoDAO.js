...
export default class ProdutoDAO {
    ...
    async gravar(produto) {
        if (produto instanceof Produto) {
            const sql = `INSERT INTO produto(prod_descricao, prod_precoCusto,
                prod_precoVenda,prod_qtdEstoque, cat_codigo)
                VALUES(?,?,?,?,?)`;
            const parametros = [produto.descricao, produto.precoCusto, produto.precoVenda, produto.qtdEstoque, produto.categoria.codigo];
    ...
    async atualizar(produto) {
        if (produto instanceof Produto) {
            ...
            , produto.qtdEstoque, produto.categoria.codigo, produto.codigo];
    ...
    async consultar(termo) {
        ...
        if (!isNaN(parseInt(termo))){
            ...
            const sql = `SELECT p.prod_codigo, p.prod_descricao,
              p.prod_precoCusto, p.prod_precoVenda,p.prod_qtdEstoque,
        ...
        else
        {
            ...
            const sql = `SELECT p.prod_codigo, p.prod_descricao,
              p.prod_precoCusto, p.prod_precoVenda, 
              p.prod_qtdEstoque,
              c.cat_codigo, c.cat_descricao
              FROM produto p INNER JOIN categoria c ON p.cat_codigo = c.cat_codigo
